...
- 支持为图片接口请求配置显式代理，网页后台生图和图片编辑同样生效
...
可在插件配置中设置 `image_proxy_url`，例如 `http://127.0.0.1:7890`。该代理只在插件请求 OpenAI 兼容图片接口并读取响应时使用，因此命令、LLM 工具和网页后台的文生图/图片编辑都会走代理；网页后台页面访问、图库读取、图片回传和提示词优化接口不会受该配置影响。
